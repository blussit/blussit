...
import {
  PhotoCapture,
  type CapturedPhoto,
} from "../../components/shared/PhotoCapture";
import {
  NowJobCard,
  type JobAction,
} from "../../components/captain/NowJobCard";
...
export default function CaptainJobsPage() {
  ...
  const beforePhotoMutation = useMutation({
    mutationFn: ({ id, photo }: { id: string; photo: CapturedPhoto }) =>
      bookingApi.captureBeforePhoto(id, photo),
    onSuccess: () => {
      invalidate();
      closeModal();
    },
  ...
  const cancelMutation = useMutation({
    mutationFn: ({ id, reason }: { id: string; reason: string }) =>
      bookingApi.captainCancel(id, reason),
    onSuccess: () => {
      invalidate();
      closeModal();
    },
  ...
  const filters = [
    { label: t("captain.status.active"), value: "" },
    { label: t("captain.status.assigned"), value: "assigned" },
    {
      label: t("captain.status.captain_on_the_way"),
      value: "captain_on_the_way",
    },
    { label: t("captain.status.service_started"), value: "service_started" },
    { label: t("captain.status.completed"), value: "completed" },
  ];
  ...
  const needsManager = (job: Booking) =>
    job.issue_flag === "captain_missed_window" && !job.issue_resolved;
  ...
  const actionFor = (job: Booking): JobAction | null => {
    ...
    if (job.status === "captain_on_the_way" && !job.vehicle_verified) {
      return { label: t("captain.actions.reachVerify"), kind: "verify" };
    }
    const map: Partial<Record<BookingStatus, JobAction>> = {
      assigned: { label: t("captain.actions.headOut"), kind: "heading" },
      captain_on_the_way: {
        label: t("captain.actions.beforePhoto"),
        kind: "before",
      },
      service_started: {
        label: t("captain.actions.afterPhoto"),
        kind: "after",
      },
    };
  ...
  return (
    <div className="space-y-5">
      ...
      {isLoading ? (
        ...
        <EmptyState
          icon={Wrench}
          title={t("captain.jobs.empty.title")}
          description={
            status === "completed"
              ? "Jobs you've completed will show up here."
      ...
      >
        ...
        <div className="mt-5 flex gap-2">
          <Button variant="outline" className="flex-1" onClick={closeModal}>
            {t("captain.common.cancel")}
          </Button>
          <Button
            className="flex-1"
            isLoading={locating || headingMutation.isPending}
      ...
      >
        <PhotoCapture
          ...
          submitError={
            beforePhotoMutation.isError
              ? getErrorMessage(beforePhotoMutation.error)
              : undefined
          }
        />
        {beforePhotoMutation.isPending && (
      ...
      >
        <PhotoCapture
          ...
          submitError={
            afterPhotoMutation.isError
              ? getErrorMessage(afterPhotoMutation.error)
              : undefined
          }
        />
        {afterPhotoMutation.isPending && (
          <p className="mt-3 text-sm text-[var(--color-text-secondary)]">
            Submitting…
          </p>
        )}
      ...
      >
        <div className="mb-3 flex items-start gap-2 rounded-lg bg-amber-50 px-3 py-2.5 text-xs text-amber-800">
          ...
          Use this when your current job is running long and you're worried you
          won't make this booking's slot on time — it notifies the manager
          immediately so they can reassign or reschedule it.
        </div>
        <textarea
