...
export function ContactUsModal({ open, onClose, returnFocusRef }: { open: boolean; onClose: () => void; returnFocusRef?: RefObject<HTMLElement | null> }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setSubmitError("");
    try {
      // Backend contract is {name, phone, email, message} — topic and
      // vehicle type ride inside the message so admins still see them.
      await contentApi.submitContactMessage({
        name: form.name.trim(), phone: form.phone.replace(/[\s-]/g, ""), email: form.email.trim(),
        message: [form.topic && `[${form.topic}]`, form.message.trim(), form.vehicle_type && `(Vehicle: ${form.vehicle_type})`]
          .filter(Boolean).join(" "),
      });
      setState("success");
    } catch (error) {
              ...
              <>
                ...
                <form className="mt-3 space-y-2 sm:mt-4 sm:space-y-3" onSubmit={submit} noValidate>
                  <div className="grid gap-2 sm:grid-cols-2 sm:gap-3">
                    ...
                    <label className={labelClass}>Email Address <span className="text-[#D99A00]">*</span><input value={form.email} onChange={(e) => update("email", e.target.value)} type="email" autoComplete="email" className={inputClass} placeholder="email@example.com" />{error(errors.email)}</label>
